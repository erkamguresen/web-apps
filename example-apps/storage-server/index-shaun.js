...
const credentials = fs
  .readFileSync(`${__dirname}/credentials.txt`, 'utf8')
  .split('\n');
...
app.post('/login', async function (req, res) {
  ...
  const credential = credentials.find((credential) => {
    ...
    return (
      ...
      hashPassword(sentPassword) === storedPassword
    );
  });
  ...
  await asyncWriteFile(
    accessTokenPath,
    JSON.stringify(storedAccessTokens, null, 2),
  );
...
app.use('/greeting', function (req, res, next) {
  if (!req.query.firstname || !req.query.lastname) {
    res
      .status(400)
      .send(
        "Both 'firstname' and 'lastname' should be present as query params",
      );
    return;
  }
...
app.get('/greeting/hello-name', function (req, res) {
  // QUERY PARAMS
...
// ?firstname=Shaun
app.get('/greeting/welcome', function (req, res) {
  // QUERY PARAMS
...
app.get('/hello-from/:city', function (req, res) {
  // ROUTE PARAMS
